...
const Field = ({ setRef = () => {}, formLinker, ...props }) => {
  ...
  const renderInput = () => {
    const commonProps = {
      ...
      error:
        formLinker.getError(props.name) &&
        !isEmpty(formLinker.getError(props.name)),
    };
    switch (props.type) {
      case "checkbox": {
        const checkProps = {
  ...
  const classes = ["form-field", props.type === "checkbox" && "checkbox-type"]
    .filter(Boolean)
    .join(" ");
  ...
  if (isNil(formLinker)) {
    ...
    console.error(
      `Warning: The ${props.name} field name is not found in the schema.`
    );
  }
...
Field.componentDescription =
  "Field is a combination of input, label, select, and error components.";
